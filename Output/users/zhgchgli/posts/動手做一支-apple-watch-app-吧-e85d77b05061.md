...
![結婚吧 — 最大婚禮籌備App](images/e85d77b05061/1*aNqsa7aR3Vi3NIIvaUFZLA.png "結婚吧 — 最大婚禮籌備App")
...
### 讓我們開始吧！
> **_這也是我第一次開發Apple Watch App，文章內容可能不夠深入，敬請大家指教！！  
> 本篇只適合有開發過iOS App/UIKit基礎的讀者閱讀  
> 本篇使用：iOS ≥ 9、watchOS ≥ 5_**  
#### 為iOS專案新建 watchOS Target:
![File -> New -> Target -> watchOS -> WatchKit App](images/e85d77b05061/1*yxwki7mCbfJbEfsTDM683A.png "File -> New -> Target -> watchOS -> WatchKit App")

_\*Apple Watch App無法獨立安裝，一定要依附在 iOS App 之下_

新建好之後目錄會長這樣：
![](images/e85d77b05061/1*WIjSrYl5Hch0mGIjlNbyFQ.png "")
...
![Scene預設會和Notification Controller Scene擠在一起 (我會把它拉上面一點分開)](images/e85d77b05061/1*2ibd9b4yaRGxwSpgKMdyUw.png "Scene預設會和Notification Controller Scene擠在一起 (我會把它拉上面一點分開)")

可在右方設定InterfaceController的標題顯示文字．

標題顏色部分吃的是Interface Builder Document/Global hint設定，整個App的風格顏色會是統一的．
![](images/e85d77b05061/1*ZcS9q4gNSBo6MZLp1eITeA.jpeg "")
#### 元件庫：
![沒有太多複雜的元件，元件功能也都簡單明瞭](images/e85d77b05061/1*Armv40CxLqJ1wlbMI_o1oQ.png "沒有太多複雜的元件，元件功能也都簡單明瞭")
#### UI 排版：
...
![Group的參數設置](images/e85d77b05061/1*aoHxAFjEGgH3ZLQx9GhH_Q.png "Group的參數設置")
...
#### 字型/字體大小設置：
![](images/e85d77b05061/1*8NfJeD4FsUw-SpAx_VFDCQ.png "")

可直接套用系統的Text Styles，或使用Custom（但這邊我測試使用Custom無法設定字體大小）；所以 **我是使用System** 自訂各顯示Label的字體大小

####  **做中學：以Line排版為例** 
![](images/e85d77b05061/1*oY9kLcnASy9j1WXxV4FGPA.png "")
...
![](images/e85d77b05061/1*bui2UXp9QwBYSYC-mwyK6g.png "")

同UIKit UITableView，有Table本體、有Cell(Apple Watch中叫做Row)；使用起來簡化許多， **你可以直接在此介面上進行Cell的設計排版！**


2. 分析排版架構，設計Row顯示樣式：
![](images/e85d77b05061/1*2bsyQ9Szfptugtg_KKxcgg.png "")

要排出一個左邊有圓角滿版的Image且堆疊一個Label，右邊平均分配上下兩個區塊，上方放Label，下方也放Label的區塊

2–1: 拉出左右兩區塊的架構
![](images/e85d77b05061/1*ez1NpEq3fgAMEqNjwTvWdw.png "")

拉兩個Group到Group中，並對Size參數分別設定：

左邊綠色部分：
![Layout設定Overlap，裡面子View要做未讀訊息Label的圖層堆疊顯示](images/e85d77b05061/1*axrBV1EHrPtOHvTnLtB79w.png "Layout設定Overlap，裡面子View要做未讀訊息Label的圖層堆疊顯示")
![設固定長寬40的正方形](images/e85d77b05061/1*Ti346bLg8AM2FInO6PNwLw.png "設固定長寬40的正方形")

右邊紅色部分：
![Layout設定Vertical，裡面子View要做上下兩個顯示](images/e85d77b05061/1*5aq_TTFEp3kq6RusiTkYcw.png "Layout設定Vertical，裡面子View要做上下兩個顯示")
![寬度設定參照外層，比例100%，扣掉左邊綠色部分40](images/e85d77b05061/1*aXH2d1kDRLNl4XsizV9P_g.png "寬度設定參照外層，比例100%，扣掉左邊綠色部分40")

左右容器內排版：
![](images/e85d77b05061/1*NR2vAZ3mqPMjCLqBCJ6ZxQ.png "")
...
![選定Row->Identifier->輸入自訂名稱](images/e85d77b05061/1*VTCVIJRAG-sGdBLjC26TKg.png "選定Row->Identifier->輸入自訂名稱")
#### Row的呈現樣式不只一種呢？

非常簡單，只要在拉一個Row放在Table裡(實際要顯示哪個樣式的ROW由程式控制)並輸入Identifier命名即可
![這邊我再拉一個Row用於呈現無資料時的提示](images/e85d77b05061/1*kQOKjxqmtI7M8BwYQ0yY0A.png "這邊我再拉一個Row用於呈現無資料時的提示")
#### 排版相關資訊

watchKit的hidden不會佔位，可拿來做交互應用（有登入才顯示Table；沒登入顯示提示Label）
![](images/e85d77b05061/1*RiCY7mH4_MyocNPN1GDuvA.png "")
...
```
![](images/e85d77b05061/1*-AnyG0_PLubAX7f-579BMw.png "")
```Swift
class ContactRow:NSObject {
    var id:String?
...
#### 如何跳頁？
![首先為Interface Controller設定Identifier](images/e85d77b05061/1*1KovG3qshPRsCgUXkbDYFw.png "首先為Interface Controller設定Identifier")
...
![push方式可左上返回](images/e85d77b05061/1*snXj8xFP0MtF3_sVWK1xUw.png "push方式可左上返回")
...
![頁籤顯示方式可以左右切換頁面](images/e85d77b05061/1*teUOM4Wql2hexR51g7v1lQ.png "頁籤顯示方式可以左右切換頁面")
...
兩端傳接收方法都ㄧ樣
![](images/e85d77b05061/1*eVT-62WCBy1ZZC90abJPqA.png "")
...
![如要修改/啟用/停用 PushNotificationPayload.apns，請選擇Target後Edit Scheme](images/e85d77b05061/1*1nlJOqwVqpMP6WtwdRcLPA.png "如要修改/啟用/停用 PushNotificationPayload.apns，請選擇Target後Edit Scheme")
...
![會同手機推播，這邊手機端iOS有實做UNUserNotificationCenter.setNotificationCategories在通知下方增加按鈕；Apple Watch預設亦然會出現](images/e85d77b05061/1*uQN8Km08rio4tylAw48LyQ.jpeg "會同手機推播，這邊手機端iOS有實做UNUserNotificationCenter.setNotificationCategories在通知下方增加按鈕；Apple Watch預設亦然會出現")
. dynamic：動態處理推播顯示樣式（重組內容、顯示圖片）
. interactive：watchOS ≥ 5 後支援，在dynamic的基礎下再增加支援按鈕

![可在Interface.storyboard中的Static Notification Interface Controller Scene設定推播處理方式](images/e85d77b05061/1*PlYKw5M3XBVDtjOa2tklgg.png "可在Interface.storyboard中的Static Notification Interface Controller Scene設定推播處理方式")

static沒什麼好說的，就是走預設的顯示方式，這邊先介紹dynamic，勾選「Has Dynamic Interface」後會出現「Dynamic Interface」可在此視圖設計你自訂的推播呈現方式（不能使用Button）：
![我的自訂推播呈現設計](images/e85d77b05061/1*RYSdWHxgmZX6Ht6m11Qpig.png "我的自訂推播呈現設計")
```Swift
import WatchKit
import Foundation
import UserNotifications
...
![左使用interactive，右使用self.notificationActions](images/e85d77b05061/1*_1Crgx61kE6F509Jd2qxPQ.jpeg "左使用interactive，右使用self.notificationActions")
...
#### 選單功能？
![在元件庫中拉入Menu，再拉入選單項目Menu Item，再拉IBAction到程式控制](images/e85d77b05061/1*qHUly8lLEa5L7FSPJCrbcw.png "在元件庫中拉入Menu，再拉入選單項目Menu Item，再拉IBAction到程式控制")

在頁面重壓就會出現：
![](images/e85d77b05061/1*9aj7kUPsv9d8XUvgCpqfOg.png "")
#### 內容輸入？
...
```
![](images/e85d77b05061/1*CWr9RIb55Sn-FoMrTmc7sQ.png "")
### 總結
>  **_謝謝你看到這！辛苦了！_** 
...
#### MurMur:
![Apple Watch App Target 部署到手錶真的有夠慢 — Narcos](images/e85d77b05061/1*-J9qZ846ZysJEhMTSZeE3w.jpeg "Apple Watch App Target 部署到手錶真的有夠慢 — Narcos")
