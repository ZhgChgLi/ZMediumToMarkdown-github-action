...
![Photo by Carlos Muza](images/9659db1357e4/1*RVPRxqz2VUuY7NGXSXzmtw.jpeg "Photo by Carlos Muza")
### 前言
#### 推播精確統計功能
...
> _詳細原理及實作方式可參考之前的文章：「_[_i_**_OS ≥ 10 Notification Service Extension 應用 (Swift)_**](ios-10-notification-service-extension-%E6%87%89%E7%94%A8-swift-cb6eba52a342)_」_
...
> _之前寫過一篇關於 Google Functions 的文章「_[_使用 Python+Google Cloud Platform+Line Bot 自動執行例行瑣事_](%E4%BD%BF%E7%94%A8-python-google-cloud-platform-line-bot-%E8%87%AA%E5%8B%95%E5%9F%B7%E8%A1%8C%E4%BE%8B%E8%A1%8C%E7%91%A3%E4%BA%8B-70a1409b149a)_」_
> _Firebase Functions 必須啟用 Blaze 專案（用多少、付多少）才能使用。_
![](images/9659db1357e4/1*YqIJ1tr2Ay-oLVjSSU0zUg.png "")
#### Firebase Firestore
...
![](images/9659db1357e4/1*5fxz4HD9q4feAqO0zXbojg.png "")

可下 `npm --version` 確認 npm 安裝成功、安裝版本：

![](images/9659db1357e4/1*VHZMRFIDzFA9AxmsDNqNlA.png "")
### 部署 Firebase Functions
#### 安裝 Firebase-tools：
```
npm install -g firebase-tools
```
![](images/9659db1357e4/1*POfMR0p1600iYqy8rzQkTQ.png "")

安裝成功後，第一次使用請先輸入：
```
firebase login
```
![](images/9659db1357e4/1*kqeECyXVPOq1cpKvcdOBeA.png "")
...
firebase init
```
![](images/9659db1357e4/1*Xx2grpX2PZb3wEFt9mQbNw.png "")
 ...
 **=== Functions Setup** 
![](images/9659db1357e4/1*2gd9pAIdLAkJRhROpJtPKA.png "")
- 語言選擇「 **JavaScript** 」
 ...
 **===Emulators Setup** 
![](images/9659db1357e4/1*xHWp195BZIZdXyUd-ub78g.png "")
...
firebase deploy
```
>  **_以後的每次修改都記得要回來下_** `firebase deploy` **_指令，才會生效。_** 

開始驗證＆部署到 Firebase…
![](images/9659db1357e4/1*hUdvD4ANKD3s73mLWNZZOQ.png "")

可能需要稍等一下，`Deploy complete!` 後你的第一個 Request & Response 網頁就完成了！


這時候可以回到 Firebase -> Functions 頁面：
![](images/9659db1357e4/1*SY4iJZL6gDEZ5AEcepIpMA.png "")

就會看到剛剛撰寫的接口和網址位置。

複製下方網址貼到 PostMan 測試：
![](images/9659db1357e4/1*OMfLkdg12QHsp-yc9RkKvA.png "")
> _POST Body 記得選擇_ `x-www-form-urlencoded` _。_
...
![](images/9659db1357e4/1*Wi-4MbPh2tVJ_utdhzN4_A.png "")
### Example Goal
...
![](images/9659db1357e4/1*yVAjhlr6wLdONeG7nY0VEw.png "")

Post Man 打成功後可以再到 Firebase -> Firestore 檢查一下資料是否有正確寫入：
![](images/9659db1357e4/1*xYVrRdFro3bQVHx05JUaTw.png "")
#### PUT 修改文章
```JavaScript
const functions = require('firebase-functions');
const admin = require('firebase-admin');
...
exports.post= functions.https.onRequest(app);
```
![](images/9659db1357e4/1*n_mI4l1EmhpWK8M_FbrzbQ.png "")
...
![](images/9659db1357e4/1*qkTMGjC0EkrMO85-6pQFwg.png "")
#### 文章按讚計數器
...
![](images/9659db1357e4/1*U9ubGe3M8XEdx9XGAV8nfA.png "")
...
exports.post= functions.https.onRequest(app);
```
![](images/9659db1357e4/1*GhNEcWUjgvYRYCMBk1DayA.png "")
...
執行完成後可看到執行結果：
![](images/9659db1357e4/1*BUcMfJJ4x_mgK0HHLc6C4g.png "")

`successful_transactions: 100` 表示 100 次都執行成功。

 **可以回 Firebase -\> Firestore 查看結果是否有 Loss Data：** 
![](images/9659db1357e4/1*wd5z743Zp9xtjKhhcMaVOg.png "")
> 成功！
#### 完整 Example Code
```JavaScript
const functions = require('firebase-functions');
...
```
#### 新增推播紀錄
![](images/9659db1357e4/1*3koe6QBxF9oOhBDqjF5mhA.png "")
#### 檢視推播統計數字
```
https://us-centra1-xxx.cloudfunctions.net/notification/iOS/1
```
![](images/9659db1357e4/1*SStEkNoDjiL7pffC2pHDkQ.png "")

另外也做了個介面統計推播數字。
#### 踩坑
> _因為對 node.js 用法不太熟悉，一開始摸索的時候在 add 資料時沒加上_ `await` _再加上寫入速度限制，導致在大流量情況下會 Data Loss…_
![](images/9659db1357e4/1*dVsBhKJQ3qqxlSvv-mCENA.png "")
### Pricing
...
![](images/9659db1357e4/1*76yRqeDyrp0kFmGHN4ZNXg.png "")
![運算時間](images/9659db1357e4/1*G_At8v80BQl81EUqPuUIbQ.png "運算時間")
![網路](images/9659db1357e4/1*iXk7oKFidHfzRVwrDvKX0A.png "網路")
...
![](images/9659db1357e4/1*ylduiqevk4WH-eNc8EOpvQ.png "")
...
- [https://firebase.google.com/docs/firestore/solutions/counters#node.js\_1](https://firebase.google.com/docs/firestore/solutions/counters#node.js_1)
